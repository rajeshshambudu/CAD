...
public class CopyAsNewFurniturePlugin extends Plugin {
  ...
  /**
   * Returns <code>true</code> if the given item may be copied.
   */
  private boolean isItemCopiable(Selectable item, Home home) {
    if (item instanceof HomePieceOfFurniture
           && ((HomePieceOfFurniture)item).isVisible()
        || item instanceof Wall) {
      return true;
    } else if (item instanceof Room) {
      // Can't copy items if selection contains only floors at the lowest level (no volume)
      Room room = (Room)item;
      if (room.isCeilingVisible()
          || room.getLevel() != null
             && room.getLevel().getElevation() != home.getLevels().get(0).getElevation()) {
        return true;
      }
    } else if (item instanceof Label) {
      // Use reflection to call getPitch which appeared only in version 5.0
      try {
        if (Label.class.getMethod("getPitch").invoke(item) != null) {
          return true;
        }
      } catch (Exception ex) {

      }
    } else if ("com.eteks.sweethome3d.model.Polyline".equals(item.getClass().getName())) {
      // Use reflection for Polyline class which appeared only in version 5.0 and isVisibleIn3D in version 6.0
      try {
        if (Boolean.TRUE.equals(item.getClass().getMethod("isVisibleIn3D").invoke(item))) {
          return true;
        }
      } catch (Exception ex) {
      }
  ...
  private Content exportSelectedItems(String pieceName) throws RecorderException {
    try {
      ...
      for (Selectable item : selectedItems) {
        if (!(item instanceof HomePieceOfFurniture)
            && isItemCopiable(item, home)) {
          root.addChild((Node)factory.createObject3D(home, item, true));
        }
      }
      // Add 3D furniture
      List<HomePieceOfFurniture> furniture = getFurniture(selectedItems);
